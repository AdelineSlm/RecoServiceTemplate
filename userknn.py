...
class UserKnn():
    ...
    def _generate_recs_mapper(self, model: ItemItemRecommender, user_mapping: Dict[int, int],
        ...
        def _recs_mapper(user):
            ...
            return [user_inv_mapping[user] for user in recs[0]], [sim for sim in recs[1]]
        return _recs_mapper
    ...
    def predict(self, test: pd.DataFrame, N_recs: int = 10):
        ...
        # recs = recs.merge(recs.groupby('user_id').agg(max_sim = ('sim', 'max')).reset_index(), on='user_id', how='left')
        # recs['sim'] = recs['sim'] / recs['max_sim']
